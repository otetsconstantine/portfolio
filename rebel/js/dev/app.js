...
let formValidate = {
  ...
  formClean(form) {
    ...
    setTimeout(() => {
      ...
      if (window["flsSelect"]) {
        let filter = form.querySelectorAll("select[data-fls-select]");
        if (filter.length) {
...
class SelectConstructor {
  ...
  selectInit(originalSelect, index) {
    ...
    if (originalSelect.options.length) {
      ...
      originalSelect.dataset.flsfilterpeed = originalSelect.dataset.flsfilterpeed ? originalSelect.dataset.flsfilterpeed : this.config.speed;
      this.config.speed = +originalSelect.dataset.flsfilterpeed;
  ...
  filterActions(e) {
    ...
    if (!isSelect && !isTag) return this.filterСlose();
  ...
  filterСlose(selectOneGroup) {
    const filterGroup = selectOneGroup ? selectOneGroup : document;
    const selectActiveItems = filterGroup.querySelectorAll(
      `${this.getSelectClass(
  ...
  selectСlose(selectItem) {
    ...
    if (!selectOptions.classList.contains("_slide")) {
      ...
      slideUp(selectOptions, originalSelect.dataset.flsfilterpeed);
      setTimeout(() => {
        selectItem.style.zIndex = "";
      }, originalSelect.dataset.flsfilterpeed);
    }
  }
  ...
  selectAction(selectItem) {
    ...
    if (originalSelect.closest("[data-fls-select-one]")) {
      ...
      this.filterСlose(selectOneGroup);
    }
    setTimeout(() => {
      if (!selectOptions.classList.contains("--slide")) {
        selectItem.classList.toggle(this.selectClasses.classSelectOpen);
        slideToggle(selectOptions, originalSelect.dataset.flsfilterpeed);
  ...
  getOptions(originalSelect) {
    ...
    +originalSelect.dataset.flsfiltercroll ? +originalSelect.dataset.flsfiltercroll : null;
  ...
  setOptionsPosition(selectItem) {
    ...
    const customMaxHeightValue = +originalSelect.dataset.flsfiltercroll ? `${+originalSelect.dataset.flsfiltercroll}px` : ``;
...
function initSliders() {
  if (document.querySelector(".slider-interesting")) {
    new Swiper(".slider-interesting", {
      ...
      */
      // Кнопки "вліво/вправо"
      navigation: {
        prevEl: ".interesting__button-prew",
        nextEl: ".interesting__button-next"
      },
      // Брейкпоінти
      breakpoints: {
        320: {
  ...
  if (document.querySelector(".slider-why")) {
    new Swiper(".slider-why", {
      // <- Вказуємо склас потрібного слайдера
      // Підключаємо модулі слайдера
      // для конкретного випадку
      modules: [Navigation],
      ...
      */
      // Кнопки "вліво/вправо"
      navigation: {
        prevEl: ".why__button-prew",
        nextEl: ".why__button-next"
      },
      // Брейкпоінти
      breakpoints: {
        320: {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (btnSearch && headerForm) {
    btnSearch.addEventListener("click", (e) => {
      e.stopPropagation();
      headerForm.classList.toggle("open");
    });
    document.addEventListener("click", (e) => {
      if (!headerForm.contains(e.target) && !btnSearch.contains(e.target)) {
